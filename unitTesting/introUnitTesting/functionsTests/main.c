 ...
 *
 * Test code contains bugs and should be complemented with more tests.
*/
...
void test_findMinMax(void)
{
...

   {
      ...
      TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
               DELTA, -1.0, result.min, "test 1");
      TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
               DELTA, -1.0, result.max, "test 2");
   }

   {
      float data2[5] = { -1.0, 0.0, 1.0, 2.0, 3.0 };
      minmax_t result = findMinMax(data2, 4);

      TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
               DELTA, 1.0, result.min, "test 3");
      TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
               DELTA, 3.0, result.max, "test 4");
   }
}
...
void test_setAverageData(void)
{
...

   {
      ...
      setAverageData(data1, 4, 2.0);
      TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
               DELTA, -2.0, averageData(data1, 5), "test 1");
   }
}
...
void test_setLimitData(void)
{
   ...
   TEST_ASSERT(
            min < max && min < 0.0 && max > 0.0);
   setLimitData(data1, 4, min, max);
   TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
            DELTA, min, findMinMax(data1, 4).min, "test 1");
   ...
   setLimitData(data1, 4, min, max);
   TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
            DELTA, max, findMinMax(data1, 4).max, "test 2");
}
